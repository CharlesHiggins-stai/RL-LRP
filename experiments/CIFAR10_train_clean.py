...
from experiments import HybridCosineDistanceCrossEntopyLoss, WrapperNet, HybridSSIMDistanceCrossEntopyLoss
...
def main():
    ...
    #     )
    typ = wandb.config.teacher_heatmap_mode
    extra_args = {
        'Experiment Class': f'VGG11 Hybrid Loss: Optimised Run {typ}'
    }   
    ...
    criterion = HybridSSIMDistanceCrossEntopyLoss(_lambda=wandb.config._lambda, mode=wandb.config.mode, step_size=wandb.config.step_size, max_lambda=wandb.config.max_lambda, min_lambda=wandb.config.min_lambda)
    if wandb.config.cpu:
...
def train(train_loader, learner_model, teacher_model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss, ssim_loss, cross_entropy_loss = criterion(heatmaps, target_maps, output, target)
        ...
        if i % wandb.config.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'ssim Loss {ssim_loss.val:.4f} ({ssim_loss.avg:.4f})\t'
                  'Cross Entropy Loss {cross_entropy_loss.val:.4f} ({cross_entropy_loss.avg:.4f})\t'
                  'loss_lambda {loss_lambda:.4f}\t'
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})'.format(
                      epoch, i, len(train_loader), batch_time=batch_time,
                      data_time=data_time, loss=losses_total, ssim_loss=losses_ssim, cross_entropy_loss=losses_cross_entropy, top1=top1, loss_lambda=criterion._lambda
                      ))
    wandb.log({
        "train/epoch": epoch,
        "train/loss": losses_total.avg,
        'train/loss_ssim': losses_ssim.avg,
    ...
    free_memory()
    del batch_time, data_time, losses_total, losses_ssim, losses_cross_entropy, top1
    del output, heatmaps, target_maps, loss, ssim_loss, cross_entropy_loss, prec1
        ...
        if i % wandb.config.print_freq == 0:
            print('Test: [{0}/{1}]\t'
                  ...
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})'.format(
                      ...
                      ssim_loss=losses_ssim, cross_entropy_loss=losses_cross_entropy,
                      top1=top1))
    wandb.log({
        "test/loss_val": losses_total.avg,
        'test/loss_ssim': losses_ssim.avg,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--_lambda', type=float, default=0.1, help='balance the loss between cross entropy and ssim distance loss')
    ...
    parser.add_argument('--teacher_checkpoint_path', type=str, help='path to teacher model checkpoint',
                        default=f"{ROOT_DIR}/baselines/trainVggBaselineForCIFAR10/save_vgg19/checkpoint_299.tar")
    parser.add_argument('--teacher_heatmap_mode', type=str, help='mode for generating teacher heatmaps, options are sanity_check, and default', default='default')
