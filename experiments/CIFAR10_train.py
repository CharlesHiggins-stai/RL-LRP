...
from experiments import HybridCosineDistanceCrossEntopyLoss, WrapperNet, WrapperNetContrastive
...
def main():
    wandb.init(project = "CIFAR10_LRP_PURE_LOSS",
        sync_tensorboard=True
        )
    extra_args = {
        'Experiment Class': 'VGG11 Pure Loss'
    }   
...
def train(train_loader, learner_model, teacher_model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            if wandb.config.teacher_heatmap_mode == 'learner_label' or wandb.config.teacher_heatmap_mode == 'learner_label_pure':
                # take the heatmaps generated by the classification of the learner model
                with torch.no_grad(): 
                    output = learner_model.model(input)
                _, target_maps = teacher_model(input, output.detach().argmax(dim=1))
            elif wandb.config.teacher_heatmap_mode == 'default' or wandb.config.teacher_heatmap_mode == 'sanity_check' or wandb.config.teacher_heatmap_mode == 'ground_truth_pure':
                # take the heatmaps generated by the teacher model on the correct classigication
        ...
        if wandb.config.teacher_heatmap_mode in ['learner_label', 'learner_label_pure', 'default', 'sanity_check']:
            # pass through and train on the argmax of the learner model
            output, heatmaps = learner_model(input)
        elif wandb.config.teacher_heatmap_mode == 'ground_truth_pure':
            # pass through and train on the correct output label of the learner model
        ...
        elif wandb.config.teacher_heatmap_mode in ['learner_label_pure', 'ground_truth_pure']:
            cosine_loss.backward()
...
def update_config_for_sweeps():
    default_args = {
        ...
        'teacher_heatmap_mode': 'learner_label_pure'
    }   
...
if __name__ == '__main__':
    ...
    parser.add_argument('--teacher_heatmap_mode', type=str, help='mode for generating teacher heatmaps, options are sanity_check, learner_label, learner_label_pure, ground_truth_target, ground_truth_pure, and default', default='ground_truth_pure')
